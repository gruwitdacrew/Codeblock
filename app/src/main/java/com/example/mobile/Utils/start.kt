...
fun start(
    ...
    dictionary: MutableMap<String, Variable> = variables
) {
    when (item[0]) {
        'i' -> {
...
//            }
            if (index != -1) {
                ...
                dictionary[name] = Variable(
                    Json.encodeToString(listNow),
                    "Array<${Translate.getType(listNow[0])}>"
                )
            } else dictionary[variable] = RPS.calculate(dictionary, expression)
        }
        ...
        '?' -> {
            ...
            if (indexOfElse.toInt() != -1) {
                val ifActions = Json.decodeFromString<List<String>>(
                    taskNow[1].substring(
                        0,
                        indexOfElse.toInt() - 1
                    )
                ...
                val elseActions =
                    Json.decodeFromString<List<String>>(taskNow[1].substring(indexOfElse.toInt()))
        ...
        '*' -> {
            ...
            val name = taskNow[1]
            val arguments =
                Json.decodeFromString<MutableList<String>>(taskNow[2].split(":", limit = 2)[0])
            ...
            dictionary[name] = Variable(
                "${Json.encodeToString(arguments).length};${Json.encodeToString(arguments)}:${
